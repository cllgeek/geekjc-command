...
function preview_basicinfo(){
    ...
    log()
    log("  "+ color.bold.rgb(10, 100, 200)(basicinfo.title + ': ' +(dt.name.val||''))  )
    log()
...
function preview_education(){
    ...
    log()
    log(" "+color.red(education.title))
    for (var i = 0; i < edu.length; i++) {
        for(var a in edu[i]){
            if(a === "timePeriod") log(),
                log("   " + color.blue('■ ') + edu[i][a].val),
                log("   ----------------------------");
            else log("   " + color.yellow('› ') + edu[i][a].info + " : " + edu[i][a].val);
        }
    };
    log()
}
        ...
        if(options.basicinfo === true && !cmd){
            ...
            for (var a in basicinfo.data) {
                if(cmd === a || a === options.basicinfo){
                    log()
                    log('   '+color.bold.rgb(10,100,200)('› '+basicinfo.data[a].info + " : ") + color.blue(basicinfo.data[a].val));
                    log()
                }
            };
        }
